  ...
  useEffect(() => {
    async function loadAll() {
      try {
        ...
        if (srv?.length) {
          // Patch known text corrections so stale Supabase data is always up to date
          const patched = srv.map(s => ({
            ...s,
            observaciones: s.observaciones
              ?.replace('Velación de 6 a 12 horas', 'Velación de 6 a 24 horas')
              ?? s.observaciones,
          }));
          setServiciosState(patched);
        }
      } catch (err) {
